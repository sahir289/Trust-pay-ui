...
const CommonTable: React.FC<CommonTableProps> = ({
  ...
  const getStatusStyles = (status: string) => {
    const statusStyles: Record<string, { color: string; icon: keyof typeof icons }> = {
  ...
  return (
    ...
    {columns?.map((col, index) => (
      <Table.Td key={index} className="py-4 font-medium border-t bg-slate-50 text-slate-500 dark:bg-darkmode-400">
          ...
          {data?.map((row, rowIndex) => (
            <Table.Tr key={rowIndex} className="[&_td]:last:border-b-0">
              {columns.map((col, colIndex) => (
                <Table.Td key={colIndex} className="py-4 border-dashed dark:bg-darkmode-600">
                  {col.type === "image" ? (
                    <Tippy
                              as="img"
                              src={row[col.key] || "https://via.placeholder.com/40"}
                              alt="Profile"
                              className="w-10 h-10 rounded-full" content={""}                    />
                  ) : col.type === "status" ? (
                    <div className={`flex items-center ${getStatusStyles(row[col.key]).color}`}>
                      <Lucide icon={getStatusStyles(row[col.key]).icon} className="w-5 h-5" />
                      {row[col.key]}
                    </div>
                  ) : col.type === "checkbox" ? (
                  ...
                  : expandable && col.type === "expand" ? (
                    ...
                    >
                      <Lucide icon={expandedRow === rowIndex ? "Minus" : "Plus"} className="w-5 h-5" />
                    </div>
                  ) : (
                    row[col.key]
                  )}
