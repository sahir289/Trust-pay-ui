...
function Main() {
  ...
  const handleSubmit = async (data: Record<string, string>) => {
    const apiData = status === Status.BANK_MISMATCH ? { type: 'PAYIN', ...data } : { ...data };
    const url = status === Status.BANK_MISMATCH ? `/update-deposit-status/${id}` : `/dispute-duplicate/${id}`;
      ...
      <div className="grid grid-cols-12 gap-6 mt-2">
        <div className="col-span-12">
          <div className="p-5 box box--stacked">
            <Tab.Group selectedIndex={parentTab} onChange={handleParentTabChange}>
